...
> الحالة الآن: الشيفرة والاختبارات وCI والحِزم وبطاقة المتجر **جاهزة** (v1.3.3، جاهز للرفع).
...
| 7. ما بعد الإطلاق | أنا | مستمر | v1.3.4 خلال أسبوعين إن ظهرت أعطال |
...
3. الإصدار 1.3.3: اللقطات من `docs/store/final/ar/` (و`en/` لتعريب الإنجليزية)، النصوص والكلمات المفتاحية من `docs/STORE_LISTING.md`، اختر بناء TestFlight.
...
2. **أنا**: وسم `v1.3.3` مطابق للبناء المرفوع، تحديث `README.md` بروابط المتجرين، ومراقبة Sentry.
...
- v1.3.4 عند تجميع 3 إصلاحات أو أي عطل يتكرر: أرفع `version` و`kAppVersion` → `Release Builds` → `Publish to Stores`.
